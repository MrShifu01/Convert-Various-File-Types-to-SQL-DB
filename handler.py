...
class Handler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        if file_path.endswith(".csv"):
            ...
            sleep(1)
            success = csv_to_sql(file_path, self.db_name, self.table_name, self.server_name)
        elif file_path.endswith(".xlsx") or file_path.endswith(".xls"):
            print(f"New Excel file {file_path} has been created!")
            sleep(1)
            success = excel_to_sql(file_path, self.db_name, self.table_name, self.server_name)
        elif file_path.endswith(".txt"):
            print(f"New Text file {file_path} has been created!")
            sleep(1)
            success = txt_to_sql(file_path, self.db_name, self.table_name, self.server_name)
        elif file_path.endswith(".json"):
            print(f"New JSON file {file_path} has been created!")
            sleep(1)
            success = json_to_sql(file_path, self.db_name, self.table_name, self.server_name)
        elif file_path.endswith(".db"):
            print(f"New DB file {file_path} has been created!")
            sleep(1)
            success = db_to_sql(file_path, self.db_name, self.table_name, self.server_name)
        elif file_path.endswith(".xml"):
            print(f"New XML file {file_path} has been created!")
            sleep(1)
            success = xml_to_sql(file_path, self.db_name, self.table_name, self.server_name)
        else:
