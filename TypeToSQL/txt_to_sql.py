...
def txt_to_sql(file_path, db_name, table_name, server_name):
    try:
        df = pd.read_csv(file_path, sep="\s+")  # assuming tab-separated values
    except Exception as e:
        print(f"Could not read Text file {file_path}: {e}")
        return False
